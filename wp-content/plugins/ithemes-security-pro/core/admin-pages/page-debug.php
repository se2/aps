	...
	public function handle_ajax_request() {
		...
		if ( false === check_ajax_referer( 'itsec-debug-page', 'nonce', false ) ) {
			...
			if ( empty( $_POST['data']['id'] ) ) {
				...
				if ( ! is_array( $data ) ) {
					ITSEC_Response::add_error( new WP_Error( 'itsec-debug-page-run-event-invalid-data', __( 'The server did not receive a valid request. The "data.data" argument for the "run_event" method is invalid JSON.', 'it-l10n-ithemes-security-pro' ) ) );
				} else {
	...
	private function get_events_table() {
		...
		<table class="widefat striped">
			...
			<tr>
				...
				<th><?php esc_html_e( 'Data', 'it-l10n-ithemes-security-pro' ) ?></th>
				<th></th>
			</tr>
			</thead>
			<tbody>
