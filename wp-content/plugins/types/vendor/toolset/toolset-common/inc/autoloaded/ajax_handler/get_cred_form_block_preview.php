...
class Toolset_Ajax_Handler_Get_Cred_Form_Block_Preview extends Toolset_Ajax_Handler_Abstract {
	 ...
	 */
	const CRED_FORMS_POST_TYPE_DICTIONARY = array(
	...
	function process_call( $arguments ) {
		...
		$cred_form_id = (int) toolset_getpost( 'formID', '_cred_form_settings' );

		if ( empty( $cred_form_id ) ) {
			$this->ajax_finish( array( 'message' => __( 'CRED form ID not set.', 'wpv-views' ) ), false );
		}

		$cred_form = get_post( $cred_form_id );

		if ( null === $cred_form ) {
			$this->ajax_finish( array( 'message' => sprintf( __( 'Error while retrieving the CRED form preview. The selected CRED form (ID: "%s") was not found.', 'wpv-views' ), $cred_form_id ) ), false );
		}

		$this->cred_form_post_type = $cred_form->post_type;

		$cred_form_content = $cred_form->post_content . '[cred-form-block-overlay form-title="' . $cred_form->post_title . '" form-id="' . $cred_form_id . '"]';

		$this->add_fake_preview_shortcodes();

		$cred_form_content = do_shortcode( $cred_form_content );
	...
	public function add_fake_preview_shortcodes() {
		...
		add_shortcode( 'cred-form-block-overlay', array( $this, 'render_cred_form_block_overlay' ) );
	}
	...
	public function remove_fake_preview_shortcodes() {
		...
		remove_shortcode( 'cred-form-block-overlay' );
	}

	public function eliminate_irrelevant_shortcodes( $atts, $content ) {
		return do_shortcode( $content );
	}

	public function render_cred_form_block_overlay( $atts ) {

		$overlay_atts = array(
			'title' => $atts['form-title'],
			'type' => 'Toolset Form',
			'link' => get_edit_post_link( $atts['form-id'] ),
		);

		ob_start();

		include_once( TOOLSET_COMMON_PATH . '/toolset-blocks/blocks/common/block-overlay.phtml' );
	...
	public function render_cred_form_field_preview( $atts ) {
		...
		if ( ! in_array( $atts['field'], $non_form_fields, true ) ) {
			...
			$field_definition_factory = Toolset_Field_Definition_Factory::get_factory_by_domain( self::CRED_FORMS_POST_TYPE_DICTIONARY[ $this->cred_form_post_type ] );
	...
	public function render_field_without_field_definition( $atts ) {
		...
		if (
			...
			if ( 'show_popular' == $atts['type'] ) {
				$field_type_slug = '';
				$output = '<span class="button disabled">' . esc_html( __( 'Add', 'wpv-views' ) ) . '</span><br />';
	 ...
	 * @param array|null $field_definition The form field definition array or null.
	...
	public function render_field( $atts, $field_type_slug, $field_definition, $button_text ) {
		...
		switch ( $field_type_slug ) {
			case 'button':
				$output = '<span class="button disabled">' . esc_html( $button_text ) . '</span>';
				break;
			case 'textfield':
			case 'numeric':
			case 'embed':
			...
			case 'audio':
			case 'video':
				$output = '<span class="button disabled">' . esc_html( __( 'Choose file', 'wpv-views' ) ) . '</span> <span>' . esc_html( __( 'No file chosen', 'wpv-views' ) ) . '</span>';
				break;
			case 'date':
