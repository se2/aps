...
Types.page.relationships.viewmodels.WizardViewModel = function(listingModel) {
	 ...
	 * @since m2m
	 */
	jQuery('#types-wizard-form').bind('reset', function() {
		// Empty setters.
		self.childPostType('');
		self.parentPostType('');
	...
	self.hasTranslatablePostType = ko.pureComputed(function() {
		...
		var parentPostTypes = postTypes.find(function(item) {return item.slug === parentType;} );
		var childPostTypes = postTypes.find(function(item) {return item.slug === childType;} );
	...
	self.onCreate = function() {
		...
		var handleSuccess = function(response, responseData) {
				...
				self.onExitWizard(function() {
					var relationship = new Types.page.relationships.viewmodels.RelationshipViewModel(responseData.updated_definitions[0], Types.page.relationships.main.viewModel.itemActions);
					Types.page.relationships.main.viewModel.items.push(relationship);
	 ...
	 */
	 if (-1 === listingModel.hooksAdded.indexOf('types-relationships-wizard-init')) {
			...
			jQuery('.wizard-limit-slider').slider({
				...
				.bind('mousedown', activateSliderSync)
				.data('sync', true)
