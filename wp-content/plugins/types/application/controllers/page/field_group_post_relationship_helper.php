 ...
 *
 * Note: This class is not covered by tests on purpose, because I expect further refactoring will be needed
 * when the Repeating field groups are implemented. After that, we should cover it with tests.
 *
 * @since m2m
 */
class Types_Page_Field_Group_Post_Relationship_Helper {
	...
	public function initialize() {
		...
		$is_association_field_group = array_key_exists( self::RELATIONSHIP_SLUG_KEY, $_GET );
	...
	/**
	 * Gets the relationsship slug, it may came from an URL parameter or from the assigned types,
	...
	private function get_returning_relationship_slug() {
		if ( ( $relationship_slug = toolset_getget( self::RELATIONSHIP_SLUG_KEY ) ) ) {
	 ...
	 * means a new field group will be created.
	 *
	...
	public function create_intermediary_group_if_needed( $group_id ) {
		$relationship_id = toolset_getget( 'relationship_id' );
		if ( $relationship_id ) {
			do_action( 'toolset_do_m2m_full_init' );
			...
			if ( $definition
				...
				$field_group_factory = Toolset_Field_Group_Factory::get_factory_by_domain( Toolset_Field_Utils::DOMAIN_POSTS );
	...
	public function get_number_associations_without_intermediary_posts() {
		...
		if ( $relationship ) {
			...
			$results = $query
				...
				->add( new Toolset_Association_Query_Condition_Empty_Intermediary() )
				->limit( 1 )
				->need_found_rows()
				->get_results();
